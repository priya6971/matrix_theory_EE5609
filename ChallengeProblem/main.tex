...
Let $\vec{v_1}$ and $\vec{v_2}$ are orthogonal vectors:

$\vec{v_1}$ and $\vec{v_2}$ are linearly independent if and only if the value of $c_1=0$, $c_2=0$ in below equation:
\begin{align}
...
Therefore we get c1 = 0 from \eqref{eq4}.
And simlarly we can proof that the value of c2 = 0 by taking dot product of vector $\vec{v_2}$ in equation \eqref{eq2}

Thus, orthogonal vectors $\vec{v_1}$ and $\vec{v_2}$ satisfy the condition of linear independence.

Similarly, we can proof that if $\vec{v_1} ,\vec{v_2}$ upto $\vec{v_n}$ are Orthogonal vectors that forms an equation \eqref{eq1}.
